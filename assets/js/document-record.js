...
window.printDocument = async function(colName, id) {
    try {
        ...
        const typeMap = { quotations: 'quotation', invoices: 'invoice', receipts: 'receipt' };
        const viewDoc = {
            type: typeMap[colName] || colName,
            data: { ...data }
        };

        localStorage.setItem('kc_view_doc', JSON.stringify(viewDoc));
        window.location.href = 'document-viewer.html';

    } catch (e) {
        console.error("Error loading document for view:", e);
